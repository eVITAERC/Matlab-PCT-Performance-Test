    ...
    methods
        ...
        function op = opConvolve(m,n,kernel,offset,mode)
            ...
            switch lower(mode)
                ...
                otherwise
                    error(['Mode parameter must be one of ''regular'','...
                        ' ''cyclic'', or ''truncated''.']);
            end

            % Check if one-dimensional convolution applies
            if (n == 1 && size(kernel,2) == 1) ||...
               (m == 1 && size(kernel,1) == 1)
                convolveOneDim = true;
                if length(offset) >= 2
                    if ((n == 1 && size(kernel,2) == 1 &&...
                ...
                convolveOneDim = false;
                if length(offset) < 2
                    error(['The offset parameter needs to contain'...
            ...
            if convolveOneDim
                ...
                if cyclic
                    ...
                    fun   = @(x,mode) opConvolveCircular1D_intrnl(...
                                                     fKernel,cflag,x,mode);
                    nRows = m;
                    nCols = m;
                else
                    ...
                    kernel = [kernel(offset:end);zeros(m-1,1);...
                              kernel(1:offset-1)];
                    if truncated
                        idx = 1:m;
                    else
                        idx = [length(kernel)-(offset-2):length(kernel),...
                               1:(m+k-offset)];
                    end
                    ...
                    % Create function handle and determine operator size
                    fun   = @(x,mode) opConvolve1D_intrnl(fKernel,k,m,...
                                                        idx,cflag,x,mode);
                    nRows = length(idx);
                    nCols = m;
                end 
                ...
                if length(offset) < 2
                    error(['Offset parameter for 2D convolution needs'...
                        ' to contain two entries.']);
                end
                offset = offset(1:2);
                ...
                if cyclic
                    ...
                    for i=0:ceil(k(1)/m)-1
                       ...
                       for j=0:ceil(k(2)/n)-1
                          ...
                          newKernel(idx1,idx2) = newKernel(idx1,idx2) +...
                                                kernel(i*m+idx1,j*n+idx2);
                       end
                    end
                    kernel = newKernel;

                    % Apply cyclic shifts to correct for offset
                    kernel = [kernel(offset(1):end,offset(2):end),...
                              kernel(offset(1):end,1:offset(2)-1);...
                              kernel(1:offset(1)-1,offset(2):end),...
                              kernel(1:offset(1)-1,1:offset(2)-1)];

                    % Precompute kernel in frequency domain
                    fKernel = fft2(full(kernel));

                    % Create function handle and determine operator size
                    fun   = @(x,mode) opConvolveCircular2D_intrnl(...
                                                fKernel,m,n,cflag,x,mode);
                    nRows = m*n;
                    nCols = m*n;
                else
                    % ========= Regular or truncated convolution =========
                    ...
                    % Shift kernel and add internal padding
                    kernel = [kernel(offset(1):end,:);...
                              zeros(m-1,k(2));...
                              kernel(1:offset(1)-1,:)];
                    kernel = [kernel(:,offset(2):end),...
                              zeros(size(kernel,1),n-1),...
                              kernel(:,1:offset(2)-1)];
                    if truncated
                        idx1 = 1:m;
                        idx2 = 1:n;
                    else
                        idx1 = [size(kernel,1)-(offset(1)-2):...
                                size(kernel,1), 1:(m+k(1)-offset(1))];
                        idx2 = [size(kernel,2)-(offset(2)-2):...
                                size(kernel,2), 1:(n+k(2)-offset(2))];
                    end
                    ...
                    % Create function handle and determine operator size
                    fun   = @(x,mode) opConvolve2D_intrnl(fKernel,k,m,n,...
                                                   idx1,idx2,cflag,x,mode);
